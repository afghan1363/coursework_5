...
class DBCreator:
    ...
    def create_tables(self):
        try:
            conn = psycopg2.connect(dbname='HeadHunter', **self.set_db_connect)
            ...
            with conn.cursor() as cur:
                for data in self.data_for_tables['employers']:
                    ...
                    cur.executemany("INSERT INTO employers VALUES (%s, %s, %s, %s)",
                                    [(employer_id, employer_name, vacancy_count, employer_url)])
            conn.commit()
            conn.close()
        except Exception:
        ...
        try:
            ...
            with conn.cursor() as cur:
                for data in self.data_for_tables['vacancies']:
                    ...
                    cur.executemany("INSERT INTO vacancies VALUES (%s, %s, %s, %s, %s, %s, %s",
                                    [(vacancy_id, employer_id, vacancy_title, description, salary_from, salary_to,
                                      vacancy_url)])
            conn.commit()
            conn.close()
        except Exception:
